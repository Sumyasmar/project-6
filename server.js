...
// To check if the server is running, you can add a simple log statement
app.get('/status', (req, res) => {
    res.json({ status: 'Server is running' });
});
// Add a new POST route to test with Postman
app.post('/postman-test', (req, res) => {
    res.json({ message: 'Postman test successful!' });
});

// 1. Get all drugs in the "Antibiotic" category
...
app.get('/drugs/names-manufacturers', (req, res) => {
    const drugNameManufacturer = drugs.map(drug => ({ name: drug.name, manufacturer: drug.manufacturer }));
    res.json(drugNameManufacturer);
});
...
app.get('/drugs/formatted', (req, res) => {
    const drugNameDose = drugs.map(drug => `${drug.name} - ${drug.dosageMg}mg`);
    res.json(drugNameDose);
});
...
app.post('/drugs/manufacturer-count', (req, res) => {
    const drugsByManufacturer = drugs.reduce((acc, drug) => {
        acc[drug.manufacturer] = (acc[drug.manufacturer] || 0) + 1;
        return acc;
    }, {});
